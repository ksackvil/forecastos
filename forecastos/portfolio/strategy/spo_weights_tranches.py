...
import forecastos.util as util
from forecastos.portfolio.constraint_model import (
    BaseConstraint,
    LongOnlyConstraint,
)
from forecastos.portfolio.cost_model import BaseCost
from forecastos.portfolio.risk_model import BaseRisk
from forecastos.portfolio.strategy import BaseStrategy
from forecastos.util import _solve_and_extract_trade_weights, get_value_at_t


class SPOWeightsTranches(BaseStrategy):
    """Optimization strategy that builds trade list using single period optimization for n_periods_held number of tranches, targeting a set of target weights.

    Each tranche is built to minimize distance from `target_weights` and is sold after n_periods_held.
    ...
    def formulate_optimization_problem(self, holdings: pd.Series, t: dt.datetime):
        ...
        # JUST CURRENT TRANCHE FOR SPO_WEIGHTS_TRANCHES
        wdiff = weights_trades - get_value_at_t(self.target_weights, t).values
